        ...
        <div class="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
            <div class="relative overflow-hidden bg-red-600 px-6 py-10 text-white sm:px-8 lg:px-10">
                ...
                <p class="mt-4 max-w-2xl text-sm leading-7 text-red-50 sm:text-base">
                    This Advanced Search System uses Laravel Scout with database indexing to search product name, SKU, and description, while supporting filters, ranked search results, and pagination.
                </p>

                <div class="mt-8 flex flex-wrap gap-3">
                    <span class="rounded-full bg-white/15 px-4 py-2 text-xs font-semibold uppercase tracking-wider">Laravel Scout</span>
                    <span class="rounded-full bg-white/15 px-4 py-2 text-xs font-semibold uppercase tracking-wider">Database Engine</span>
        ...
        @if ($products->count())
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                @foreach ($products as $product)
                    <article class="card-rise overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <span class="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-slate-700">
                                        {{ ucfirst($product->status) }}
                                    </span>
                                </div>

                                <h4 class="mt-4 text-xl font-black tracking-tight text-slate-900">
                                    {{ $product->name }}
                                </h4>
